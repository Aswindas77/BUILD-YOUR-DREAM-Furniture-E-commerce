...
const verifyUser = async (req, res) => {
  try {
    ...
    if (user.isBlocked) {
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ emailError: "sorry you have been blocked!" });
    }
  ...
  try {
    ...
    if (checkData.trim() === "signUp") {
      ...
      return res
        ...
        .send("forgot password otp verified successfully");
    }
...
const verifyOtp = async (req, res) => {
  try {
    ...
    if (String(otp) !== String(sessionOtp)) {
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ message: "Invalid OTP or Invalid format" });
    }

    if (currentTime - otpSentTime > 60000) {
      return res
        .status(HttpStatus.NOT_FOUND)
        .json({ message: "Your OTP has expired" });
    }
    ...
    if (checkData == "singup") {
      ...
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ message: "Invalid session state" });
    }
  } catch (err) {
    console.error(err);
    res
...
const googleLogin = async (req, res) => {
  try {
    ...
    if (user.isBlocked) {
      req.session.gerrmessage =
        "Your google account has been blocked. Please contact support.";
      const categories = await Category.find({ isDeleted: false });
...
const buyNow = async (req, res) => {
  try {
    ...
    const dummyId = `ORD-${Date.now().toString().slice(-6)}-${Math.floor(
    ...
    if (paymentMethod.toLowerCase() === "paypal") {
      ...
      const { orderId, paypalRedirectUrl } = await createPayPalOrder(
        totalAmount,
        userId
      );
      console.log(
        paypalRedirectUrl,
        "paypal order inside paypal approvalLinkapprovalLinkapprovalLink"
      );
      return res.status(HttpStatus.OK).json({
        success: true,
...
const filterProducts = async (req, res) => {
  try {
    ...
    const products = await Products.find(query)
      ...
      .populate("category")
      .select(
        "name description salesPrice productOffer stock images status ratings isDeleted isListed createdAt updatedAt"
      );
    ...
    res
      .status(HttpStatus.INTERNAL_SERVER_ERROR)
      .render("error", { message: "Error filtering products" });
  }
};
...
const loadWhishList = async (req, res) => {
  try {
    ...
    const cartProductIds = cart
      ? cart.products.map((p) => p.productId._id.toString())
      : [];

    const wishlist = await WhishList.findOne({ userId: user._id }).populate({
...
const deleteWhishlist = async (req, res) => {
  try {
    ...
    if (!whishlist || !whishlist.products || whishlist.products.length === 0) {
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ success: false, message: "Whishlist is empty" });
    }
    ...
    if (productIndex === -1) {
      return res
        .status(HttpStatus.NOT_FOUND)
        .json({ success: false, message: "Product not found in whishlist" });
    }
...
const placePendingOrder = async (req, res) => {
  try {
    ...
    res
      .status(HttpStatus.INTERNAL_SERVER_ERROR)
      .json({ success: false, message: "Failed to place pending order." });
  }
};
  ...
  try {
    ...
    if (!phoneRegex.test(phone)) {
      return res.status(HttpStatus.BAD_REQUEST).json({
        success: false,
        message:
          "Invalid phone number format. Must be a 10-digit number starting with 6-9.",
      });
    }
