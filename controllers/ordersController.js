...
const updateReturnStatus = async (req, res) => {
  try {
    ...
    if (status === "Approved") {
      ...
      await Orders.findByIdAndUpdate(returnRequest.orderId._id, {
        status: "Returned",
    ...
    res.json({
      ...
      message: `Return ${status.toLowerCase()} successfully${
        status === "Approved" ? " and refund processed" : ""
      }`,
    });
  } catch (error) {
...
const generateInvoice = async (req, res) => {
  try {
    ...
    doc
      .font("Helvetica-Bold")
      .fontSize(24)
      .text("BUILD YOUR DREAM", 200, 45, { align: "right" });
    doc
      .font("Helvetica")
      .fontSize(10)
    ...
    doc
      ...
      .text(
        `House/Building: ${selectedAddress.houseNumber}`,
        330,
        invoiceDetailsY + 65
      )
      .text(
