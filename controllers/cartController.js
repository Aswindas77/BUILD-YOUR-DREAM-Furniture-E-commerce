  ...
  try {
    ...
    if (!product.stock > 0) {
      return res
        ...
        .json({
          success: false,
          message: "the products is currently unavailable",
        });
    }
...
const deleteCart = async (req, res) => {
  try {
    ...
    return res
      .status(HttpStatus.OK)
      .json({ success: true, message: "Product removed from cart", newTotal });
  } catch (error) {
    res
      .status(HttpStatus.INTERNAL_SERVER_ERROR)
      .json({ message: Messages.INTERNAL_ERROR });
...
const loadCheckout = async (req, res) => {
  try {
    ...
    const userAddresses = await Address.findOne({ userId: userId }).populate(
      "userId"
    );
...
const renderCheckoutPage = async (req, res) => {
  try {
    ...
    const [user, cart, categories, userAddresses] = await Promise.all([
